...
(defun web-help (site url)
  (browse-url (search-site-url site url
                (thing-at-point 'symbol))))

(defun google () (interactive)
  (browse-url
    (concat "http://www.google.com/search?"
      (format "q=%s" (url-hexify-string (thing-at-point 'symbol))))))

(defun javahelp () (interactive)
  (web-help "java.sun.com" "/javase/6/docs/api/"))

(defun rubyhelp () (interactive)
  (web-help "www.ruby-doc.org" "/core/"))

(global-set-key [(f1)] 'google)
(global-set-key [(f2)] 'javahelp)
(global-set-key [(f3)] 'rubyhelp)
